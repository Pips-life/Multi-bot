...
class BotListener extends SynchronizationListener{
 ...
 onPositionUpdated(instanceIndex,p){if(p?.symbol===SYMBOL)reconcile();}
 onPositionRemoved(){reconcile();}
 onOrderUpdated(instanceIndex,o){if(o?.symbol===SYMBOL)reconcile();}
 onOrderCompleted(){reconcile();}
 onOrderFailed(instanceIndex,id,error){setStatus(`Order failed: ${error?.message||error}`);reconcile();}
}

async function connectSdk(){
 if(connecting||connection)return;const token=cleanToken(ui.token.value),accountId=ui.account.value.trim();
 if(!token||token==='SAVED TOKEN'){setStatus('MetaAPI token is missing');return;}if(!validAccountId(accountId)){setStatus('MetaAPI account ID format is invalid');return;}
 connecting=true;setStatus('Connecting directly to MetaApi…');
 try{const C=sdkConstructor();api=new C(token);account=await api.metatraderAccountApi.getAccount(accountId);if(!account?.id)throw new Error('MetaApi account not found');if(typeof account.waitConnected==='function')await account.waitConnected();connection=account.getStreamingConnection();listener=new BotListener();connection.addSynchronizationListener(listener);await connection.connect();await connection.waitSynchronized();synchronized=true;await connection.subscribeToMarketData(SYMBOL);reconcile();setStatus('CONNECTED — XAUUSD live stream active');}
 catch(e){const msg=e?.message||String(e);try{if(connection)await connection.close();}catch(_){}try{if(api)await api.close();}catch(_){}connection=null;account=null;api=null;synchronized=false;setStatus(`MetaAPI connection failed: ${msg}`);}finally{connecting=false;}
}
...
async function ensureStop(position,mid){
 ...
 try{
   ...
   if(result?.stringCode&&result.stringCode!=='TRADE_RETCODE_DONE')throw new Error(result.message||result.stringCode);
   await new Promise(r=>setTimeout(r,20));reconcile();
...
async function reconcile(){
 if(!connection?.terminalState)return;const ps=positions(),os=orders();
...
async function enter(side){
 if(entryInFlight||positions().length>=MAX_POSITIONS)return;const volume=currentVolume();if(volume<=0)return;entryInFlight=true;
...
async function onTick(mid,bid,ask,previous){
 await reconcile();const ps=positions();
...
const savedToken=cleanToken(localStorage.getItem('metaapi.token')),savedAccount=String(localStorage.getItem('metaapi.accountId')||'').trim();if(savedToken&&savedAccount){ui.token.value=savedToken;ui.account.value=savedAccount;connectSdk().finally(()=>{ui.token.value='SAVED TOKEN';ui.token.disabled=true;});}
