...
const DIRECTION_SCORE = 0.72;

// Profit protection: +100 pips arms intrabar reversal monitoring.
// It is an activation threshold, NOT a take-profit target.
const MOMENTUM_ARM_PIPS = 100;
const REVERSAL_LOOKBACK_TICKS = 5;
const REVERSAL_MIN_ADVERSE_PIPS = 3;
const REVERSAL_MIN_PEAK_RETRACE_PIPS = 5;
...
let lastEntryPrice=NaN, signalSide='', signalStreak=0;
const priceHistory=[];
...
function directionSignal(){
  if(priceHistory.length<SLOW_EMA+5)return {side:'',score:0};
  const mids=priceHistory.map(x=>x.mid), fast=ema(mids.slice(-FAST_EMA*2),FAST_EMA), slow=ema(mids.slice(-SLOW_EMA*2),SLOW_EMA), oldFast=ema(mids.slice(-(FAST_EMA*2+5),-5),FAST_EMA), recent=mids.slice(-12);
  let ups=0,downs=0;for(let i=1;i<recent.length;i++){if(recent[i]>recent[i-1])ups++;else if(recent[i]<recent[i-1])downs++;}
  const pip=brokerPipSize(), consistency=Math.max(ups,downs)/Math.max(1,recent.length-1), slopePips=(fast-oldFast)/pip, separationPips=(fast-slow)/pip, netPips=(recent.at(-1)-recent[0])/pip;
  const bullish=fast>slow&&slopePips>0.8&&netPips>2&&ups>downs, bearish=fast<slow&&slopePips<-0.8&&netPips<-2&&downs>ups;
  const score=Math.min(1,consistency*.45+Math.min(1,Math.abs(separationPips)/8)*.25+Math.min(1,Math.abs(slopePips)/4)*.2+Math.min(1,Math.abs(netPips)/12)*.1);
  return {side:bullish?'BUY':bearish?'SELL':'',score,consistency,slopePips,separationPips,netPips};
}
function confirmedDirection(){const s=directionSignal();if(!s.side||s.score<DIRECTION_SCORE){signalSide='';signalStreak=0;return s;}if(signalSide===s.side)signalStreak++;else{signalSide=s.side;signalStreak=1;}return s;}
...
function reversalDetected(state,side,reference,pip){
  ...
  for(let i=1;i<state.prices.length;i++){
    const d=(state.prices[i]-state.prices[i-1])/pip;
    if(side==='BUY'){if(d<0)adverseMoves++;if(d>0)favorableMoves++;}
    else {if(d>0)adverseMoves++;if(d<0)favorableMoves++;}
  }
  ...
  return adverseMoves>=3 && adverseMoves>favorableMoves && netAgainst>=REVERSAL_MIN_ADVERSE_PIPS && adverseFromPeak>=REVERSAL_MIN_PEAK_RETRACE_PIPS;
}
async function manageMomentumExits(positions,reference,bid,ask,now){
  ...
  for(const p of positions){
    ...
    let state=momentumExitState.get(id);
    const currentCandle=candleId(now);
    ...
    if(reversalDetected(state,side,reference,brokerPipSize())){
      state.exitInFlight=true;
      setStatus(`QUICK EXIT — ${side} momentum reversed after +${pp.toFixed(1)} pips`);
      try{await connection.closePosition(id);}catch(e){state.exitInFlight=false;setStatus(`Quick exit failed: ${e?.message||e}`);}
    }
  }
}
...
async function onTick(mid,bid,ask,previous){const positions=ownedPositions();if(positions.length)await manageMomentumExits(positions,mid,bid,ask,Date.now());reconcile();if(Number.isNaN(previous)){setStatus('Streaming XAUUSD — building direction model');return;}const signal=confirmedDirection();if(!signal.side){if(!ownedPositions().length)setStatus(`WAITING — direction not confirmed | samples ${priceHistory.length}/${SLOW_EMA+5}`);return;}if(signalStreak<MIN_CONFIRMATIONS){if(!ownedPositions().length)setStatus(`CONFIRMING ${signal.side} — ${signalStreak}/${MIN_CONFIRMATIONS} | score ${Math.round(signal.score*100)}%`);return;}const livePositions=ownedPositions();if(livePositions.length>=MAX_POSITIONS){setStatus(`HOLDING ${livePositions.length}/${MAX_POSITIONS} ${signal.side} | SL ${SL_PIPS} | reversal exit armed at +${MOMENTUM_ARM_PIPS}`);return;}if(livePositions.length&&livePositions.some(p=>sideOf(p)!==signal.side))return;await enter(signal.side,bid,ask,signal);}
...
function changeCredentials(){trading=false;stopForegroundService();localStorage.removeItem('metaapi.token');localStorage.removeItem('metaapi.accountId');if(connection)connection.close().catch(()=>{});if(api)api.close().catch(()=>{});connection=null;account=null;api=null;synchronized=false;ui.token.disabled=false;ui.token.value='';ui.account.value='';priceHistory.length=0;signalSide='';signalStreak=0;lastEntryPrice=NaN;momentumExitState.clear();setStatus('Enter new MetaAPI credentials');}
function startBot(){if(!connection||!synchronized){setStatus('Connect MetaApi first');return;}startForegroundService();trading=true;setStatus('BOT RUNNING — robust direction detection + 100-pip momentum reversal exits active');}
