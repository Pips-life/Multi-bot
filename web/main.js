...
const FAST_EMA = 8;
const SLOW_EMA = 21;
const QUICK_LOOKBACK_TICKS = 6;
const QUICK_MIN_DIRECTIONAL_TICKS = 4;
const QUICK_MIN_NET_PIPS = 1.5;
const QUICK_MIN_VELOCITY_PIPS = 0.30;
const QUICK_MIN_ACCELERATION_PIPS = 0.05;
const MIN_ENTRY_SPACING_PIPS = 15;
...
function isOurs(x){return !!x&&x.symbol===SYMBOL&&(Number(x.magic)===MAGIC||String(x.clientId??'').startsWith('MB'))};
...
function directionSignal(){
  ...
  const oldFast=ema(mids.slice(-(FAST_EMA*2+5),-5),FAST_EMA);
  const slopePips=(fast-oldFast)/pip;
  const bias=fast>slow?'BUY':fast<slow?'SELL':'';
  const bullish=ups>=QUICK_MIN_DIRECTIONAL_TICKS&&netPips>=QUICK_MIN_NET_PIPS&&velocityPips>=QUICK_MIN_VELOCITY_PIPS&&(accelerationPips>=QUICK_MIN_ACCELERATION_PIPS||slopePips>=0.5);
  const bearish=downs>=QUICK_MIN_DIRECTIONAL_TICKS&&netPips<=-QUICK_MIN_NET_PIPS&&velocityPips<=-QUICK_MIN_VELOCITY_PIPS&&(accelerationPips<=-QUICK_MIN_ACCELERATION_PIPS||slopePips<=-0.5);
  const side=bullish?'BUY':bearish?'SELL':'';
  const consistency=Math.max(ups,downs)/Math.max(1,recent.length-1);
  const velocityScore=Math.min(1,Math.abs(velocityPips)/1.5);
  const accelerationScore=Math.min(1,Math.abs(accelerationPips)/2);
  const biasScore=side&&bias===side?1:0;
  const score=Math.min(1,consistency*.45+velocityScore*.25+accelerationScore*.20+biasScore*.10);
...
// MetaApi restricts the combined comment + clientId length. Keep both short and deterministic.
function tradeOptions(side){const clientId=`MB${Date.now().toString(36).slice(-9)}`;return {comment:side==='BUY'?'B':'S',magic:MAGIC,clientId};}
