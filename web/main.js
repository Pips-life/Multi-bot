...
async function enter(side,bid,ask,signal){const positions=ownedPositions();if(entryInFlight||positions.length>=MAX_POSITIONS||!connection||!synchronized||!positionDistanceAllows(side,positions))return;const volume=currentVolume(),reference=side==='BUY'?ask:bid,{stopLoss,takeProfit}=protectiveLevels(side,reference);if(!Number.isFinite(stopLoss)||!Number.isFinite(takeProfit))return;entryInFlight=true;try{const clientId=`MB_${Date.now().toString(36)}_${Math.floor(Math.random()*36).toString(36)}`,options=tradeOptions(`MB ${side}`,clientId);if(side==='BUY')await connection.createMarketBuyOrder(SYMBOL,volume,stopLoss,takeProfit,options);else await connection.createMarketSellOrder(SYMBOL,volume,stopLoss,takeProfit,options);lastEntryPrice=reference;setStatus(`OPEN ${side} ${volume} — FIXED SL ${SL_PIPS} / TP ${TP_PIPS}`);await waitForPosition(side,5000);reconcile();}catch(e){setStatus(`Entry failed: ${e?.message||e}`);}finally{entryInFlight=false;}}
