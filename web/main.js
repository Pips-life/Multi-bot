...
function logEvent(text,level='INFO'){eventLog.push({time:new Date(),text:String(text),level});if(eventLog.length>100)eventLog.shift();renderLogs();}
function setStatus(text){if(text!==lastStatus){lastStatus=text;ui.status.textContent=text;if(text!==lastLoggedStatus){lastLoggedStatus=text;logEvent(text,/failed|error|disconnected/i.test(text)?'ERROR':'INFO');}}}
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
function renderOpenTrades(){
 const body=$('openTradesBody'),count=$('tradeCount');if(!body)return;const positions=ownedPositions();if(count)count.textContent=`${positions.length} / ${MAX_POSITIONS}`;
 if(!positions.length){body.innerHTML='<tr><td colspan="8" class="emptyState">No active Multi-bot positions</td></tr>';return;}
 body.innerHTML=positions.map(p=>{const side=sideOf(p),pp=profitPips(p,lastMid),profit=Number(p.profit);return `<tr><td class="${side==='BUY'?'sideBuy':'sideSell'}">${side}</td><td>${esc(p.volume)}</td><td>${fmt(p.openPrice)}</td><td>${fmt(lastMid)}</td><td>${fmt(p.stopLoss)}</td><td>${Number(p.takeProfit)>0?fmt(p.takeProfit):'Dynamic'}</td><td class="${pp>=0?'profitPos':'profitNeg'}">${Number.isFinite(pp)?pp.toFixed(1):'—'}</td><td class="${profit>=0?'profitPos':'profitNeg'}">${Number.isFinite(profit)?profit.toFixed(2):'—'}</td></tr>`;}).join('');
}
function renderHistory(){
 const body=$('historyBody');if(!body)return;const deals=(connection?.historyStorage?.deals||[]).filter(d=>isOurs(d)&&String(d.entryType||'').toUpperCase().includes('OUT')).sort((a,b)=>Date.parse(b.time||0)-Date.parse(a.time||0)).slice(0,50);
 if(!deals.length){body.innerHTML='<tr><td colspan="6" class="emptyState">No closed Multi-bot deals found</td></tr>';return;}
 body.innerHTML=deals.map(d=>{const side=String(d.type||'').toUpperCase().includes('SELL')?'SELL':'BUY',profit=Number(d.profit);const t=d.time?new Date(d.time).toLocaleString([], {month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}):'—';return `<tr><td>${esc(t)}</td><td class="${side==='BUY'?'sideBuy':'sideSell'}">${side}</td><td>${esc(d.volume)}</td><td>${fmt(d.price)}</td><td class="${profit>=0?'profitPos':'profitNeg'}">${Number.isFinite(profit)?profit.toFixed(2):'—'}</td><td>${esc(d.id)}</td></tr>`;}).join('');
}
function renderLogs(){
 const body=$('logBody'),count=$('logCount');if(!body)return;if(count)count.textContent=`${eventLog.length} events`;if(!eventLog.length){body.innerHTML='<div class="emptyState">No events yet</div>';return;}
 body.innerHTML=[...eventLog].reverse().map(e=>`<div class="logRow ${e.level==='ERROR'?'logError':''}"><span class="logTime">${e.time.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'})}</span><span class="logLevel">${esc(e.level)}</span><span class="logText">${esc(e.text)}</span></div>`).join('');
}
...
renderLogs();
function bindUi(){ui.save.onclick=()=>{localStorage.setItem('metaapiToken',cleanToken(ui.token.value));localStorage.setItem('metaapiAccount',ui.account.value.trim());setStatus('Credentials saved on device');};ui.change.onclick=()=>{ui.token.value='';ui.account.value='';localStorage.removeItem('metaapiToken');localStorage.removeItem('metaapiAccount');setStatus('Credentials cleared');};ui.start.onclick=async()=>{logEvent('START BOT requested');if(trading||connecting)return;signalSide='';lastMid=NaN;priceHistory.length=0;trading=false;startForegroundService();setStatus('START BOT — connecting…');const ok=await connectSdk();if(!ok){stopForegroundService();return;}trading=true;setStatus('BOT STARTED — scanning quick XAUUSD momentum');};ui.stopBot.onclick=async()=>{trading=false;stopForegroundService();setStatus('BOT STOPPED');logEvent('Trading loop stopped by user');};ui.token.value=localStorage.getItem('metaapiToken')||'';ui.account.value=localStorage.getItem('metaapiAccount')||'';}
bindUi();
