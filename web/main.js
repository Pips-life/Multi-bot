...
function updateMarketPhase(mid){
  ...
  const c=workingCandle,pip=brokerPipSize(),disp=Math.max(Math.abs(c.close-c.open),Math.abs(mid-biasAnchor))/pip;
  ...
  if(reversalCandidate){
    ...
    else marketPhase=`DEEP RETRACEMENT — ${directionBias} BIAS`;return;
  }
  if(retracing){marketPhase=`RETRACE/PAUSE — HOLD ${directionBias}`;return;}
...
function entrySignal(mid){
  if(!directionBias||!Number.isFinite(lastMid)||retracing||reversalCandidate)return '';
  const c=workingCandle;if(!c)return '';
  const pip=brokerPipSize(),move=directionBias==='BUY'?mid-lastMid:lastMid-mid;
...
async function onTick(mid,bid,ask){if(!trading||stopRequested||!synchronized)return;if(!currentPosition){const signal=entrySignal(mid);if(signal)await enter(signal,signal==='BUY'?ask:bid);}else await trail(bid,ask);if(currentPosition)setStatus(`Running ${sideOf(currentPosition)} | ${marketPhase} | SL ${fmt(Number(currentPosition.stopLoss??0))}`);else if(directionBias)setStatus(`WAITING | ${marketPhase} | bias only`);lastMid=mid;}
...
addLog('Pips-life engine loaded — previous-candle displacement bias + retracement/change detection + 70-pip trailing SL');renderPageData();