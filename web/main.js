...
const REVERSAL_CONFIRMATIONS=3;
...
async function enter(side,bid,ask){const positions=ownedPositions();if(entryInFlight||positions.length>=MAX_POSITIONS||!connection||!synchronized||!positionDistanceAllows(side,positions))return;if(Date.now()-lastTradeActionAt<TRADE_COOLDOWN_MS){setStatus(`COOL-OFF ${Math.max(0,TRADE_COOLDOWN_MS-(Date.now()-lastTradeActionAt))}ms before next trade`);return;}const volume=currentVolume(),reference=side==='BUY'?ask:bid,stopLoss=trailingStop(side,reference);if(!Number.isFinite(stopLoss))return;entryInFlight=true;try{const clientId=`MB_${Date.now().toString(36)}_${Math.floor(Math.random()*36).toString(36)}`,options={comment:`MB ${side}`,magic:MAGIC,clientId};if(side==='BUY')await connection.createMarketBuyOrder(SYMBOL,volume,stopLoss,undefined,options);else await connection.createMarketSellOrder(SYMBOL,volume,stopLoss,undefined,options);lastEntryPrice=reference;lastTradeActionAt=Date.now();setStatus(`OPEN ${side} ${volume} — 3s COOL-OFF | BE @ ${BREAK_EVEN_ARM_PIPS}p | PROFIT LOCK`);await waitForPosition(side,5000);reconcile();}catch(e){setStatus(`Entry failed: ${e?.message||e}`);}finally{entryInFlight=false;}}
...
async function onTick(mid,bid,ask,previous){reconcile();if(Number.isNaN(previous)){setStatus('Streaming XAUUSD — building direction model');return;}const positions=ownedPositions();if(positions.length){const heldSide=sideOf(positions[0]);const adversePrice=heldSide==='BUY'?bid:ask;const profits=positions.map(p=>profitPips(p,adversePrice)).filter(Number.isFinite);const weakestProfit=profits.length?Math.min(...profits):NaN;if(momentumFaded(heldSide)){const count=(reversalCounts.get(heldSide)||0)+1;reversalCounts.set(heldSide,count);const protectedProfit=Number.isFinite(weakestProfit)&&weakestProfit>=BREAK_EVEN_ARM_PIPS;const confirmed=count>=REVERSAL_CONFIRMATIONS;if(protectedProfit&&confirmed){await closeOurs(`CONFIRMED REVERSAL ${heldSide} +${weakestProfit.toFixed(0)}p`);reversalCounts.clear();return;}setStatus(`HOLDING ${positions.length}/${MAX_POSITIONS} ${heldSide} | reversal ${count}/${REVERSAL_CONFIRMATIONS} | lock ${protectedProfit?'ARMED':'WAIT '+BREAK_EVEN_ARM_PIPS+'p'}`);}else{reversalCounts.delete(heldSide);setStatus(`HOLDING ${positions.length}/${MAX_POSITIONS} ${heldSide} | spread ${SPREAD_PIPS}p | BE ${BREAK_EVEN_ARM_PIPS}p | profit lock`);}if(positions.length>=MAX_POSITIONS)return;}const signal=directionSignal();if(!signal.side){if(!positions.length)setStatus(`WAITING — building direction | samples ${priceHistory.length}/${SLOW_EMA+2}`);return;}if(positions.length&&positions.some(p=>sideOf(p)!==signal.side))return;await enter(signal.side,bid,ask);}
