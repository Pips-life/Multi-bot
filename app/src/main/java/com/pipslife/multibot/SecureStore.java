...
public final class SecureStore {
    private static final String PREFS = "multibot_secure";
    private static final String KEY_ALIAS = "multibot_token_key";
    ...
    public void save(String accountId, String token) throws Exception {
        Cipher c = Cipher.getInstance("AES/GCM/NoPadding"); c.init(Cipher.ENCRYPT_MODE, key());
        byte[] iv = c.getIV(); byte[] enc = c.doFinal(token.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        prefs.edit().putString("account", accountId).putString("token", Base64.encodeToString(iv, Base64.NO_WRAP) + "." + Base64.encodeToString(enc, Base64.NO_WRAP)).apply();
    }
    public String accountId() { return prefs.getString("account", ""); }
    public String token() throws Exception {
        String packed = prefs.getString("token", ""); if (packed.isEmpty()) return "";
        ...
        return new String(c.doFinal(Base64.decode(p[1], Base64.NO_WRAP)), java.nio.charset.StandardCharsets.UTF_8);
    }
