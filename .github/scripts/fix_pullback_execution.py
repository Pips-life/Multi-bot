...
# Remove any older candle/range quality gate.
if "function marketQuality(){" in s:
    start = s.index('function marketQuality(){')
    end = s.index('\nfunction updateBias()', start)
    s = s[:start] + s[end + 1:]

# Canonical entry decision: direction bias + pullback/velocity only.
...
# Make broker specification access defensive. A bad/temporarily unavailable
# specification must not throw out of the entry path.
...
ns, count = re.subn(stop_pattern, stop, s, count=1, flags=re.S)
if count != 1:
    raise SystemExit('Pips-life SL patch could not locate setInitialStop()')
s = ns

# Ensure every detected position independently receives protection, and retry
# missing protection on the next reconciliation rather than leaving it exposed.
...
print('Entry validation hardened and every open position now gets an independently confirmed broker-side SL with retries.')
