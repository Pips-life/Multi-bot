...
# Apply the opposite-stop trail to the generated trading bundle.
...
async function handleTriggeredTrail(order){
 const oid=orderIdOf(order);if(!oid||triggeredTrailOrders.has(oid))return;let sourceId='';for(const [positionId,state] of oppositeTrailOrders.entries()){if(state?.orderId===oid){sourceId=positionId;break;}}if(!sourceId)return;triggeredTrailOrders.add(oid);oppositeTrailOrders.delete(sourceId);const source=ownedPositions().find(p=>idOf(p)===sourceId);if(source){try{await connection.closePosition(sourceId);setStatus(`REVERSE ${sourceId.slice(-6)} — old ${sideOf(source)} closed`);}catch(e){setStatus(`REVERSE CLOSE ${sourceId.slice(-6)} failed — ${e?.message||e}`);}}}
