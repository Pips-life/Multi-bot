...
s = s.replace("lastEntryPrice=reference;setStatus(`OPEN BATCH ${side} ${slots} × ${volume} — SL ${INITIAL_SL_PIPS}p | TP ${TAKE_PROFIT_PIPS}p`);", "lastEntryPrice=reference;setStatus(`OPEN BATCH ${side} ${marginSlots} × ${volume} — margin-aware | SL ${INITIAL_SL_PIPS}p | TP ${TAKE_PROFIT_PIPS}p`);")
...
newtick = """async function onTick(mid,bid,ask,previous){
  ...
  if(positions.length){
    ...
    }else setStatus(`HOLDING ${positions.length}/${MAX_POSITIONS} ${heldSide} | ${pnlPips.toFixed(0)}p | SL ${INITIAL_SL_PIPS}p | TP ${TAKE_PROFIT_PIPS}p`);
