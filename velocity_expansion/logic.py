...
class VelocityExpansionLogic:
    ...
    When the STOP fills, close the source position, reverse in the stop
    direction, and install a fresh 100-pip opposite STOP.
    """
    ...
    def on_tick(self, bid: float, ask: float, timestamp: datetime, fills: list[Fill] | None = None) -> list[Fill]:
        """Trail on every live XAUUSD tick, then process STOP fills."""
    ...
    def _handle_stop_fill(self, fill: Fill, timestamp: datetime) -> None:
        ...
        result = self.execution.submit(OrderRequest(
            ...
            timestamp=timestamp, metadata={"velocity_expansion_reversal": True, "trail_pips": TRAIL_PIPS},
        ))
        if result.status == "FILLED" and result.fill is not None:
            self._install_stop(result.fill, timestamp)
